...
import { getPeriodKey } from "@/src/shared/lib/date";
 ...
 * Groups by custom period when cycle config is provided.
...
export async function getMonthlyTotals(
  ...
  // Calculate date range — go extra months back to account for cycle offset
  ...
  for (const t of data ?? []) {
    ...
    const key = getPeriodKey(d, cycleDay, cycleHour);
